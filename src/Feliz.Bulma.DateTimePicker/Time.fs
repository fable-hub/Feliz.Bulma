...
module TimePicker =
    ...
    let private safeMinutes (mins: int) =
        if mins < 0 then
            59
        else if mins > 59 then
            0
        else
            mins

    [<ReactComponent>]
    let internal timePickerValue
        (cn: string)
        (fixFn: int -> int)
        (count: int)
    ...
    let internal timePickerHours (defVal: int) (onDataChanged: int -> unit) =
        timePickerValue "timepicker-hours" safeHours defVal onDataChanged

    let internal timePickerMinutes (defVal: int) (onDataChanged: int -> unit) =
        timePickerValue "timepicker-minutes" safeMinutes defVal onDataChanged

    let internal toTimeSpans (hs, ms, he, me) =
        TimeSpan.FromHours(float hs).Add(TimeSpan.FromMinutes(float ms)),
        TimeSpan.FromHours(float he).Add(TimeSpan.FromMinutes(float me))

    let internal toHrsMins (ts: TimeSpan, te: TimeSpan) =
        ts.Hours, ts.Minutes, te.Hours, te.Minutes
    ...
    let timePicker (p: Props) =
        ...
        let updateTime (t: (TimeSpan * TimeSpan) option) =
            ...
            if
                p.isRange
                && p.onTimeRangeSelected.IsSome
            then
                t |> p.onTimeRangeSelected.Value

            if not p.isRange && p.onTimeSelected.IsSome then
                t
                |> Option.map fst
                |> p.onTimeSelected.Value
        ...
        let txtValue =
            if p.isRange then
                time
                |> Option.map (fun (s, t) ->
                    sprintf "%s - %s" (toFormattedTime s) (toFormattedTime t)
                )
            else
                time
                |> Option.map fst
                |> Option.map toFormattedTime
            |> Option.defaultValue ""
...
[<Erase>]
type timePicker =
    static member inline onTimeSelected
        (eventHandler: TimeSpan option -> unit)
        : ITimePickerProperty
        =
        unbox ("onTimeSelected", eventHandler)

    static member inline onTimeRangeSelected
        (eventHandler: (TimeSpan * TimeSpan) option -> unit)
        : ITimePickerProperty
        =
        unbox ("onTimeRangeSelected", eventHandler)

    static member inline onShow(eventHandler: unit -> unit) : ITimePickerProperty =
        unbox ("onShow", eventHandler)

    static member inline onHide(eventHandler: unit -> unit) : ITimePickerProperty =
        unbox ("onHide", eventHandler)

    static member inline defaultValue(v: TimeSpan) : ITimePickerProperty = unbox ("defaultValue", v)

    static member inline defaultRangeValue(v: (TimeSpan * TimeSpan)) : ITimePickerProperty =
        unbox ("defaultRangeValue", v)

    static member inline isRange(v: bool) : ITimePickerProperty = unbox ("isRange", v)

    static member inline displayMode(v: DisplayMode) : ITimePickerProperty =
        unbox ("displayMode", v)
