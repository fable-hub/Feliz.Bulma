                    ...
                    (fun (event: Event) ->
                        if
                            (isNullOrUndefined ref.current
                             || ref.current.Value.contains (event.target :?> _))
                        then
                            ()
                        else
                            handler (event)
...
module TagsInputComponent =
    ...
    let input (p: Props) =
        ...
        let changeTags value =
            ...
            if p.onTagsChanged.IsSome then
                p.onTagsChanged.Value value
            ...
            async {
                let filter vals =
                    if p.removeSelectedFromAutoComplete then
                        let isSelected v =
                            tags
                            |> List.exists (fun x -> String.Compare(x, v, not p.caseSensitive) = 0)
        ...
        let addTag (isFromDropdown: bool) (t: string) =
            ...
            let isDuplicate =
                tags
                |> List.exists (fun x -> String.Compare(x, t, not p.caseSensitive) = 0)

            let isInAutoComplete =
                autoComplete
                |> List.exists (fun x -> String.Compare(x, t, not p.caseSensitive) = 0)

            let canBeAdded =
                (String.IsNullOrWhiteSpace(t) |> not)
                && (isDuplicate && p.allowDuplicates
                    || not isDuplicate)
                && (isInAutoComplete
                    && p.allowOnlyAutoCompleteValues
                    || not p.allowOnlyAutoCompleteValues)
        ...
        let autoCompleteItems =
            ...
            | true, _ ->
                Bulma.dropdownItem.div [
                    prop.className "empty-title"
                    prop.text p.loadingLabel
                ]
                |> List.singleton
            | false, [] ->
                Bulma.dropdownItem.div [
                    prop.className "empty-title"
                    prop.text p.noResultsLabel
                ]
                |> List.singleton
            | false, l ->
                l
                |> List.map (fun x ->
                    Bulma.dropdownItem.a [
                        prop.text x
                        prop.onClick (fun _ -> addTag true x)
                    ]
                )

        React.useEffect (
            loadAutoComplete >> Async.StartImmediate,
            [|
                box inputValue
                box isDropdownShown
                box tags
        ...
        let onKeyDown (k: KeyboardEvent) =
            if k.key.ToUpperInvariant() = "ENTER" then
                ...
                inputValue.Replace(p.delimiter, Char.MinValue)
                |> addTag false
...
[<Erase>]
type tagsInput =
    static member inline onTagsChanged(fn: string list -> unit) : ITagsInputProperty =
        unbox ("onTagsChanged", fn)

    static member inline autoCompleteSource
        (src: string -> Async<string list>)
        : ITagsInputProperty
        =
        unbox ("autoCompleteSource", src)

    static member inline value(tags: string list) : ITagsInputProperty = unbox ("value", tags)

    static member inline defaultValue(tags: string list) : ITagsInputProperty =
        unbox ("defaultValue", tags)

    static member inline noResultsLabel(label: string) : ITagsInputProperty =
        unbox ("noResultsLabel", label)

    static member inline loadingLabel(label: string) : ITagsInputProperty =
        unbox ("loadingLabel", label)

    static member inline delimiter(value: char) : ITagsInputProperty = unbox ("delimiter", value)

    static member inline placeholder(value: string) : ITagsInputProperty =
        unbox ("placeholder", value)

    static member inline allowDuplicates(value: bool) : ITagsInputProperty =
        unbox ("allowDuplicates", value)

    static member inline caseSensitive(value: bool) : ITagsInputProperty =
        unbox ("caseSensitive", value)

    static member inline closeDropdownOnItemSelect(value: bool) : ITagsInputProperty =
        unbox ("closeDropdownOnItemSelect", value)

    static member inline tagsRemovable(value: bool) : ITagsInputProperty =
        unbox ("tagsRemovable", value)

    static member inline tagProperties(props: IReactProperty list) : ITagsInputProperty =
        unbox ("tagProperties", props)

    static member inline autoTrim(value: bool) : ITagsInputProperty = unbox ("autoTrim", value)

    static member inline removeSelectedFromAutoComplete(value: bool) : ITagsInputProperty =
        unbox ("removeSelectedFromAutoComplete", value)
